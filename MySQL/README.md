...
添加主键（Primary key）通常是优化查询的一种有效方式，但是关于主键也有一些注意事项要知道。
...
*MySQL* 检索出的数据**并不是以纯粹的随机顺序显示**，而是将以它在底层中表现出来的顺序显示（比如添加到表中的顺序）。但是如果后来进行了更新或删除操作，那么此顺序将会收到影响。因此，如果不明确控制的话，不能依赖该排序顺序。

关系数据库设计理论认为：**如果不明确排序顺序，则不应该假定检索出的数据的顺序有意义**。

## 应用层过滤的缺点

这里应用层指的一般是编程语言（像 Java、Python 等等）。

- 极大的影响应用的性能
- 不具备可伸缩性
- 网络带宽浪费

> 说明：应用层过滤是指不使用 *MySQL* 自身的过滤机制（如 `WHERE`、`LIKE` 等），而是靠自身的条件判断对数据进行**二次过滤的行为**。
